...
import Untitled.StuffFactory;
import Untitled.world.Tile;
import Untitled.world.World;
import Untitled.world.WorldBuilder;
...
public class PlayScreen implements Screen {
    ...
    private void createItems(StuffFactory factory) {
        ...
        factory.newRelic(world.depth() - 1);
    }
        ...
        if (subscreen != null) {
            ...
            switch (key.getKeyCode()) {
                ...
                case KeyEvent.VK_W:
                    ...
                    break;
                case KeyEvent.VK_X:
                    subscreen = new InspectScreen(player);
                    break;
                ...
                case KeyEvent.VK_F:
                    ...
                    break;
                case KeyEvent.VK_Q:
                    subscreen = new DrinkScreen(player);
                    break;
            }
            ...
            switch (key.getKeyChar()) {
                ...
                case ',':
                    ...
                    for (Creature c : world.getCreatures()) {
                        if (c.glyph() != '@' && c.z == player.z)
                            toDelete.add(c);
                    }
                    world.getCreatures().removeAll(toDelete);
                    break;
                case '<':
                ...
                case '>':
                    if (!areCreaturesDead()) {
                        player.notify("You must kill all enemies before you can proceed!");
                        break;
                    }
                    player.moveBy(0, 0, 1);
                    break;
    ...
    private boolean areCreaturesDead() {
        for (Creature c : world.getCreatures()) {
            if (c.z != player.z) {
                continue;
            }
    ...
    private Screen userExits() {
        for (Item item : player.inventory().getItems()) {
            if (item != null && item.name().equals("Golden Relic"))
                return new EndScreen(player);
        }
        return new EndScreen(player);
    }
