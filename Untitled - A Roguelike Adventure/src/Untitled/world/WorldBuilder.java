...
public class WorldBuilder {
    ...
    public WorldBuilder connectRooms(){
        for(int i = 0; i < rooms.size() - 3; i += 3){
            int startz = rooms.get(i);
            int startx = rooms.get(i+1);
            int starty = rooms.get(i+2);

            int endz = rooms.get(i+3);
            int endx = rooms.get(i+4);
            int endy = rooms.get(i+5);

            if (startz  == endz){
                while (startx != endx){
                    tiles[startx][starty][startz] = Tile.FLOOR;
    ...
    public WorldBuilder makeFloors() {
